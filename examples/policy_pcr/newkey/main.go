...
func main() {
	...
	trialSession, err := tpmaead.NewPCRSession(sel, tpm2.TPM2BDigest{Buffer: pcrHash})
	if err != nil {
		fmt.Printf("go-kms-wrapping:  Could not get PCRMap: %s", err)
		return
	}

	kfs, err := tpmaead.NewKey(*tpmPath, tpmaead.AESKey256, nil, []byte(*parentPass), trialSession)
	if err != nil {
		fmt.Printf("go-kms-wrapping:  Could not get PCRMap: %s", err)
		return
	}
